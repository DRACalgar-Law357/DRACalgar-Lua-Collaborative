...
--Register events
function spikeball.onInitAPI()
	npcManager.registerEvent(npcID, spikeball, "onTickEndNPC")
	npcManager.registerEvent(npcID, spikeball, "onDrawNPC")
end

function spikeball.onTickEndNPC(v)
	...
	if not data.initialized then
		data.lockedFrame = v.animationFrame -- used to lock the spikeball's frame upon falling
	...
	if v.collidesBlockBottom then -- handles moving on the ground
		...
		if v.collidesBlockLeft or v.collidesBlockRight then -- kills the spikeball upon touching any wall.
	...
	for _,n in NPC.iterateIntersecting(v.x - 3,v.y - 3, v.x + v.width + 3, v.y + v.height + 3) do -- handles killing other npcs & spikeballs bumping into each other
		if n.id ~= v.id then
			n:harm(3)
