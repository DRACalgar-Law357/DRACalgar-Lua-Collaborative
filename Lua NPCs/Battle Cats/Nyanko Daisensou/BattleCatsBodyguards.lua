...
function NPCBodyguards.onTickEndNPC(v)
	...
	if v.collidesBlockBottom then
		if data.KBCount <= 0 then
			data.stunned = false
			data.KBCount = 0
		else
			data.KBCount = data.KBCount - 1
			v.speedY = -3.75
		end
	end
	if data.stunned == false then
		if data.state == 0 then
			if v.dontMove == false then
				if v.collidesBlockBottom then
					...
					if data.animateState ~= 1 then changeAnimateState(v,data,config,1) end
					...
					v.speedX = 0
					if data.animateState ~= 2 then changeAnimateState(v,data,config,2) end
				end
			else
				data.state = 1
				v.speedX = 0
				if data.animateState ~= 0 then changeAnimateState(v,data,config,0) end
			end
		elseif data.state == 1 then
			...
			if v.collidesBlockBottom then
				if data.animateState ~= 1 and v.speedX ~= 0 then changeAnimateState(v,data,config,1) end
				if data.animateState ~= 0 and v.speedX == 0 then changeAnimateState(v,data,config,0) end
			else
				if data.animateState ~= 2 then changeAnimateState(v,data,config,2) end
			end
			...
			if v.ai2 == 0 then
				...
				if (data.timer >= NPC.config[v.id].smokeDelay and data.state == 2) or (data.timer >= NPC.config[v.id].fragDelay and data.state == 3) then
					...
					if data.state == 2 then
						...
						n = NPC.spawn(NPC.config[v.id].smokeGrenade, v.x + throwSpawnOffset[v.direction], vy + NPC.config[v.id].throwY, player.section, false, true)
						...
						n = NPC.spawn(NPC.config[v.id].fragGrenade, v.x + throwSpawnOffset[v.direction], vy + NPC.config[v.id].throwY, player.section, false, true)
				...
				if data.animateState ~= 3 and data.state == 3 then changeAnimateState(v,data,config,3) end
				if data.animateState ~= 5 and data.state == 2 then changeAnimateState(v,data,config,5) end
			else
				v.speedX = 0
				...
				if data.animateState ~= 4 and data.state == 3 then changeAnimateState(v,data,config,4) end
				if data.animateState ~= 6 and data.state == 2 then changeAnimateState(v,data,config,6) end
			end
		elseif data.state == 4 then
			...
			elseif v.ai2 == 1 or v.ai2 == 2 then
					...
					if (v.ai2 == 1 and v.ai1 < NPC.config[v.id].lowShootDuration) or (v.ai2 == 2 and v.ai1 < NPC.config[v.id].highShootDuration) then
						...
						if v.ai2 == 1 then
							...
							for i=1,NPC.config[v.id].lowShotSpawnConsecutive do
								p = NPC.spawn(NPC.config[v.id].lowShot, v.x + lowShotSpawnOffset[v.direction], vy + NPC.config[v.id].lowShootY, player.section, false, true)
							...
							for i=1,NPC.config[v.id].highShotSpawnConsecutive do
								p = NPC.spawn(NPC.config[v.id].highShot, v.x + highShotSpawnOffset[v.direction], vy + NPC.config[v.id].highShootY, player.section, false, true)
								p.direction = v.direction
				...
				if data.animateState ~= 8 and v.ai2 == 1 then changeAnimateState(v,data,config,8) end
				if data.animateState ~= 7 and v.ai2 == 2 then changeAnimateState(v,data,config,7) end
			else
				v.speedX = 0
		...
		elseif data.state == 5 then
			if v.ai3 == 0 then
				...
				if data.timer < NPC.config[v.id].punchDelay then
					if v.collidesBlockBottom then
						if data.animateState ~= 12 then changeAnimateState(v,data,config,12) end
					else
						if data.animateState ~= 2 then changeAnimateState(v,data,config,2) end
					end
				else
					if v.collidesBlockBottom then
						if data.animateState ~= 13 then changeAnimateState(v,data,config,13) end
					else
						if data.animateState ~= 14 then changeAnimateState(v,data,config,14) end
					end
				end
				if data.timer == NPC.config[v.id].punchDelay then
				...
				v.speedX = 0
				if data.animateState ~= 15 then changeAnimateState(v,data,config,15) end
		...
		elseif data.state == 6 then
			v.speedX = 0
			if v.ai2 == 0 then
				if data.animateState ~= 9 then changeAnimateState(v,data,config,9) end
				...
				if data.timer >= NPC.config[v.id].backupDelay then
					...
					Routine.setFrameTimer(settings.backupFrequency, (function() 
						local p = NPC.spawn(settings.backupID, v.spawnX + v.width/2 + settings.backupX, v.spawnY + v.height/2 + settings.backupY, v.section, true, true)
				...
				if data.animateState ~= 17 then changeAnimateState(v,data,config,17) end
		...
		elseif data.state == 7 then
			if data.animateState ~= 10 then changeAnimateState(v,data,config,10) end
		...
		elseif data.state == 8 then
			...
			elseif v.ai2 == 1 then
				...
				if data.animateState ~= 16 and v.speedX ~= 0 then changeAnimateState(v,data,config,16) end
			else
				v.speedX = 0
				if data.animateState ~= 18 then changeAnimateState(v,data,config,18) end
		...
		if data.animateState ~= 11 then changeAnimateState(v,data,config,11) end
	end
	if data.heal ~= nil then
	...
	if data.iFrames then
		...
		if v.collidesBlockBottom and (data.state == 6 or data.state == 7) and data.health > 0 then
			if data.hurtTimer == 1 then
				v.speedY = -5.6
				v.speedX = RNG.irandomEntry{1.75,2.5} * -v.direction
				data.stunned = true
				data.timer = 0
				data.walkTimer = 0
				data.KBCount = 1
	...
	if Colliders.collide(plr, v) and not v.friendly and not data.stunned and not Defines.cheat_donthurtme then
		plr:harm()
    end
...
function NPCBodyguards.onNPCHarm(eventObj, v, reason, culprit)
	...
	if reason ~= HARM_TYPE_LAVA then
		if reason == HARM_TYPE_JUMP or killReason == HARM_TYPE_FROMBELOW then
			SFX.play(2)
			data.iFrames = true
