...
function sampleNPC.onTickEndNPC(v)
	...
	if not data.initialized then
		...
		data.statelimit = -1
		data.swordConsecutive = 3
		data.necessaryShield = false
		data.waveTimer = 0
		data.randomShieldDelay = RNG.randomInt(0,64)
		data.pinch = true
		data.randomDashBack = RNG.randomInt(0,1)
	end
	...
	if data.health <= NPC.config[v.id].hp/2 and data.state == STATE_IDLE then
		data.swordConsecutive = 5
		data.pinch = true
	end
	if data.state == STATE_IDLE then
		...
		if data.timer == 1 then
			...
			if data.necessaryShield == false then
				local options = {}
				if data.statelimit ~= 0 then table.insert(options,0) table.insert(options,0) table.insert(options,0) end
				...
				if data.pinch == false then
					v.ai2 = RNG.randomInt(0,1)
				else
					v.ai2 = RNG.randomInt(0,2)
				end
				data.randomDashBack = RNG.randomInt(0,1)
			end
		end
		if (data.timer >= 56 and data.necessaryShield == false) or (data.timer >= 40 and data.necessaryShield == true) then
			data.timer = 0
			if data.necessaryShield == false then
				if v.ai5 == 0 then
					if v.ai2 == 0 then
	...
	elseif data.state == STATE_RAM then
		if v.ai1 == 0 then
			if data.timer <= 130 then
				...
				if data.timer >= 270 or (((v.direction == -1 and plr.x > v.x + v.width/2) or (v.direction == 1 and plr.x < v.x + v.width/2)) and data.randomDashBack == 1) then
					data.timer = 0
					v.ai1 = 2
				end
		...
		elseif v.ai1 == 1 then
			...
			v.animationFrame = 19
			v.speedX = v.speedX - 0.09 * v.direction
			...
			if math.abs(v.speedX) <= 0.1 then
				v.speedX = 0
				data.timer = 0
				if data.randomDashBack == 1 then
					v.ai1 = 0
					data.timer = 120
					data.randomDashBack = 0
					npcutils.faceNearestPlayer(v)
				else
					data.state = STATE_IDLE
					v.ai1 = 0
	...
	elseif data.state == STATE_SWORD then
		if v.ai3 == 0 then
			if v.ai1 == 0 then
				...
				--Slash four times; if in a pinch, slash six times
		...
		elseif v.ai3 == 2 then
			if v.ai1 == 0 then
				...
				if data.timer == 1 then SFX.play(13) end
				if data.timer >= 50 then v.ai1 = 1 data.timer = 0 v.speedX = 3.3 * v.direction v.speedY = -6 end
			elseif v.ai1 == 1 then
				--Animation for sword beam attack and charging
			...
			if data.timer == 1 then SFX.play("Undertale Beam Charge.ogg") end
			if Colliders.collide(plr,data.swordBox) and (v.animationFrame == 5 or v.animationFrame == 6) then
				plr:harm()
			end
			if data.timer == 62 then
				...
				if #footCollisions > 0 then
					collidesWithSolid = true
					if not data.lastFrameCollision then
						local id = NPC.config[v.id].swordBeamID
						--Sword Beam rotation formula: f.data.rotation = ((math.pi*1.5)+((math.pi*0.5)*v.direction))%(math.pi*2)
						for i=0,2 do
							local angleinc = 30
							if v.direction == -1 then angleinc = 30 end
							if v.direction == 1 then angleinc = -30 end
							local f = NPC.spawn(id, v.x + spawnOffset[v.direction], footCollisions[1].y - 0.5 * NPC.config[id].height, v:mem(0x146, FIELD_WORD), false, true)
							f.direction = v.direction
							if f.direction == DIR_RIGHT then
								f.data.rotation = i * -11.7
							else
								f.data.rotation = 180 + (i * 11.7)
							end
							f.speedX = 4 * f.direction
							f.speedY = i * -1.5
						end
						return
					end
	...
	elseif data.state == STATE_SHIELD then
		...
		if data.timer < 12 or data.timer >= 100 + data.randomShieldDelay then
			v.animationFrame = 17
		else
			v.animationFrame = 18
		end
		if data.timer >= 110 + data.randomShieldDelay then
			data.state = STATE_IDLE
			data.timer = 0
			data.necessaryShield = false
			data.randomShieldDelay = RNG.randomInt(0,64)
		...
		if reason ~= HARM_TYPE_LAVA then
			...
			if data.health <= 0 then
				...
				for _,n in ipairs(NPC.get()) do
					if n.id == NPC.config[v.id].swordBeamID or n.id == NPC.config[v.id].shockwaveID then
