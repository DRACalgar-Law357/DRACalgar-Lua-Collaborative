...
function cryoBlaster.onTickEndNPC(v)
	...
	if not data.initialized then
		...
		data.prop1img = data.prop1img or Sprite{x = 0, y = 0, pivot = vector(0.5, 0.5), frames = frames, texture = config.prop1Image}
		data.prop2img = data.prop2img or Sprite{x = 0, y = 0, pivot = vector(0.5, 0.5), frames = frames, texture = config.prop2Image}
		data.angle = 0
...
function cryoBlaster.onDrawNPC(v)
	...
	if v.legacyBoss == true and data.state ~= STATE.KILL and data.state ~= STATE.KAMIKAZE and data.health then
		Graphics.drawImage(hpboarder, 740, 120, 0, 0, 28, 168)
