...
local frankSettings = {
    ...
    puddleImg = {
        texture = Graphics.loadImageResolved("npc-"..npcID.."-puddle.png"),
        cord = {
            [-1] = {x=0,y=40},
            [1] = {x=0,y=40},
        },
...
function frank.onTickEndNPC(v)
    ...
    elseif data.state == STATE.QUAKE then
        if v.ai1 == 0 then
            if data.timer == 1 and #data.bgoTable <= 0 then data.timer = 0 v.ai1 = 1 data.timer = config.debrisDelay return end
            v.speedX = 0
    ...
    elseif data.state == STATE.HURT then
        v.animationFrame = 11
        v.speedX = 0
        data.sweatTimer = data.sweatTimer + 1
        v.sweatFrame = math.floor(data.sweatTimer / config.sweatImg.framespeed) % config.sweatImg.frames
        if data.timer >= config.hurtDelay then
            data.timer = 0
            if data.health <= 0 then
                if data.temperatureState == 2 then
                    data.state = STATE.MELT
                else
    ...
    elseif data.state == STATE.MELT then
        v.friendly = true
        if data.timer == 1 and v.meltY ~= config.gfxheight then v.meltY = config.gfxheight-config.gfxoffsety end
        v.speedX = 0
        v.animationFrame = 12
        v.meltY = math.clamp(v.meltY - config.meltRate,0,config.gfxheight-config.gfxoffsety)
        if v.meltY <= config.gfxheight/2 then
            data.puddle = true
            data.puddleTimer = math.clamp(data.puddleTimer + 0.5, 0, config.puddleImg.height)
        end
        if data.timer >= config.evaporateDelay and data.timer % 4 == 0 then
            local a = Animation.spawn(10,v.x+v.width/2-16,v.y+v.height-16)
            a.speedX = RNG.random(-3,3)
            a.speedY = RNG.random(-2,-4)
        end
        if data.timer >= config.meltDelay then
            v:kill(9)
        end
	end
    ...
    if v.meltY > 0 and data.state == STATE.MELT then
		data.melting = true
	else
		data.melting = false
	end
...
function frank.onDrawNPC(v)
    ...
    if data.state == STATE.MELT then
        ...
        if v.meltY and v.meltY > 0 and v.animationFrame >= 0 and data.melting == true then
            Graphics.drawImageToSceneWP( --melt animation
                Graphics.sprites.npc[v.id].img,
                v.x + 0.5 * v.width + config.spriteoffsetx + config.meltAnimation.x,
                v.y + 0.5 * v.height + config.spriteoffsety-v.meltY + config.meltAnimation.y,
                0,
                v.animationFrame * config.gfxheight,
                config.gfxwidth,
                v.meltY,
                1,
                p
        ...
        if data.puddle == true and config.puddleImg.texture then
            Graphics.drawImageToSceneWP( --puddle animation
            config.puddleImg.texture,
            v.x+v.width/2-config.puddleImg.width/2+config.puddleImg.cord[v.direction].x,
            v.y+v.height/2-config.puddleImg.height/2-data.puddleTimer+config.puddleImg.cord[v.direction].y,
            0,
            0,
            config.puddleImg.width,
            data.puddleTimer,
            1,
            p-0.1
        )
        ...
        npcutils.hideNPC(v)
        return
    elseif data.state == STATE.HURT and config.sweatImg.texture then
        local p = config.priority
            Graphics.drawImageToSceneWP( --sweat animation
            config.sweatImg.texture,
            v.x+v.width/2-config.sweatImg.width/2+config.sweatImg.cord[v.direction].x,
            v.y+v.height/2-config.sweatImg.height/2+config.sweatImg.cord[v.direction].y,
            0,
            config.sweatImg.height * v.sweatFrame,
            config.sweatImg.width,
            config.sweatImg.height,
            1,
            p-0.1
        )
    end
