...
-- cooldown (the delay before the orbiting NPCs disappear)
...
function Orbit.new(args)
	...
	t.cooldown = args.cooldown or 720
		...
		if centerX == nil then
			...
			if not orbit.initialized then
				...
				for i = 1, orbit.number do
					...
					local a = Animation.spawn(10,centerX + radiusX*math.cos(rotationCounter), centerY + radiusY*math.cos(rotationCounter),orbit.section, true, true)
			...
			if orbit.cooldown > 0 then
				orbit.cooldown = orbit.cooldown - 1;
			else
				orbit:destroy(true);
			end
			...
			for i=#orbit.orbitingNPCs, 1, -1 do
				...
				if npc.isValid then
					...
					if ((player.holdingNPC ~= nil) and (player.holdingNPC == npc)) or npc:mem(0x138, FIELD_WORD) ~= 0 or npc.id == 263 then
