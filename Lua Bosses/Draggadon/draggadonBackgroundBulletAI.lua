...
backgroundBills.fireBreathID = 872
backgroundBills.fireRainID = 873
...
do
    ...
    function backgroundBills.onCameraDrawBlaster(v,camIdx)
        ...
        v.animationFrame = math.floor(lunatime.tick() / config.framespeed) % config.frames
...
local preventRegister = {} --A table that holds manual NPC registrations. 1 = blocklist, -1 = bgolist
--Prevents the bullet from reaching further by blocks
function backgroundBills.blocklistNPC(ids)
	for _,v in ipairs(ids) do
		table.insert(preventRegister,v,1)
	end
end

--Prevents the bullet from reaching further by BGOs
function backgroundBills.bgolistNPC(ids)
	for _,v in ipairs(ids) do
		table.insert(preventRegister,v,-1)
	end
end
--backgroundBills.blocklistNPC({40,411})
backgroundBills.bgolistNPC({79,301,302,303,75,76,77,78,89,90,91,47,139,140,146,314,260,261,315,318,319,320,322,323,335,336,324,325,326,321,353,285,286,281,282,283,291,292,293,284,287,288,289,290,17,16,327,328,329,330,331,332,333,334,122,123,124,118,121,871,872})
    ...
    function initialiseBullet(v,data,config,settings)
        ...
        data.beNotFriendly = data.beNotFriendly
    ...
    function backgroundBills.onTickBullet(v)
        ...
        then return end
        if v.id == backgroundBills.fireBreathID then
            ...
            if data.depth < config.hitboxDepth*0.5 + 1 then
                for k, b in Block.iterateIntersecting(v.x, v.y, v.x + v.width, v.y + v.height) do
                    if preventRegister[b.id] == 1 or Block.config[b.id].SIZEABLE then
                        v:kill(HARM_TYPE_NPC)
                        SFX.play(3)
                        data.beNotFriendly = false
                        break
                    end
                end
                for _,bgo in ipairs(BGO.getIntersecting(v.x, v.y, v.x + v.width, v.y + v.height)) do
                    if preventRegister[bgo.id] == -1 then -- If this is a redirector and has a speed associated with it
                        v:kill(HARM_TYPE_NPC)
                        SFX.play(3)
                        data.beNotFriendly = false
                    end
            ...
            if data.depth < config.hitboxDepth*0.5 then
                if data.beNotFriendly == true then
                    v.friendly = false
                end
            else
                v.friendly = true
            end
        elseif v.id == backgroundBills.fireRainID then
