...
function sampleNPC.onTickNPC(v)
	...
	v.despawnTimer = 8
	if v.y <= camera.y + sampleNPCSettings.screenTopY then
		local relocate = RNG.irandomEntry(data.bgoTable)
		local ball = NPC.spawn(426,relocate.x+22,relocate.y+40,v:mem(0x146,FIELD_WORD),false,true)
		ball.direction = -1
		if ball.x + ball.width/2 <= camera.x + camera.width/2 then ball.direction = 1 end
		ball.speedX = 2 * ball.direction
		ball.speedY = 0
		v:kill(9)
	end
end
